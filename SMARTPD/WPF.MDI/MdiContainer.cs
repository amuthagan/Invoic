...
namespace WPF.MDI
{
    ...
    public class MdiContainer : UserControl
    {
        ...
        /// </summary>
        private double _windowOffsetLeft;
        private double _windowOffsetTop;
        ...
        public bool IsFormAlreadyOpen(string formtitle)
        {
            try
            {
                ...
                containerCount = Children.Count;
                if (containerCount == 1) return false;
                for (int i = 0; i < containerCount; i++)
                {
                    containertitle = Children[i].Title.ToString().ToUpper();

                    if (containertitle.CompareTo(formtitle.ToUpper()) == 0)
                    {
                        count = count + 1; ;
                    }
                }
        ...
        public MdiChild GetFormAlreadyOpened(string formtitle)
        {
            try
            {
                int containerCount = 0;
